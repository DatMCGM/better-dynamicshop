...
app.get('/api/servers/:serverId/items/:material/transactions', (req, res) => {
  try {
    const hours = parseInt(req.query.hours) || 24;
    const since = Date.now() - hours * 3600000;
    res.json(db.getItemTransactions(req.params.serverId, req.params.material.toUpperCase(), since));
  } catch (err) { res.status(500).json({ error: err.message }); }
});
